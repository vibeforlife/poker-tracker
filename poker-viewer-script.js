...
function showNightDetail(nightId) {
  ...
  sorted.forEach(function(a, i) {
    ...
    html += '<span style="font-weight: bold; color: ' + (a.winnings >= 0 ? '#2ecc71' : '#e74c3c') + '">
...
loadData();
 + a.winnings.toFixed(2) + '</span>';
    html += '</div>';
  });
  
  // Add Settlement Section
  html += '<h3 style="margin-top: 30px;">💸 Settlement</h3>';
  html += '<div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px; margin-top: 15px;">';
  
  // Calculate poker settlement
  var pokerTrans = [];
  var balances = night.attendees.map(function(a) {
    return { player: appData.players.find(function(p) { return p.id === a.playerId; }).name, balance: a.winnings };
  ...
  debtors.forEach(function(debtor) {
    ...
    creditors.forEach(function(creditor) {
      if (remaining > 0.01 && creditor.amount > 0.01) {
        var amt = Math.min(remaining, creditor.amount);
        pokerTrans.push({ from: debtor.player, to: creditor.player, amount: amt });
        remaining -= amt;
        creditor.amount -= amt;
      }
    });
  ...
  if (night.expenses && night.expenses.paidBy && night.expenses.splitAmong && night.expenses.splitAmong.length > 0) {
    ...
    if (totalExp > 0) {
      ...
      var paidByPlayer = appData.players.find(function(p) { return p.id === night.expenses.paidBy; });
      night.expenses.splitAmong.forEach(function(pid) {
        if (pid !== night.expenses.paidBy) {
          ...
          if (p) expenseTrans.push({ from: p.name, to: paidByPlayer.name, amount: perPerson });
        }
      });
    }
  }
  ...
  allTrans.forEach(function(t) {
    if (!netBal[t.from]) netBal[t.from] = 0;
    if (!netBal[t.to]) netBal[t.to] = 0;
    netBal[t.from] -= t.amount;
    netBal[t.to] += t.amount;
  });
  
  var finalDebtors = [];
  var finalCreditors = [];
  Object.keys(netBal).forEach(function(person) {
    var bal = Math.round(netBal[person] * 100) / 100;
    if (bal < -0.01) finalDebtors.push({ person: person, amount: -bal });
    else if (bal > 0.01) finalCreditors.push({ person: person, amount: bal });
  });
  ...
  while (i < finalDebtors.length && j < finalCreditors.length) {
    var amt = Math.min(finalDebtors[i].amount, finalCreditors[j].amount);
    if (amt > 0.01) {
      optimized.push({ from: finalDebtors[i].person, to: finalCreditors[j].person, amount: Math.round(amt * 100) / 100 });
    }
    finalDebtors[i].amount -= amt;
    finalCreditors[j].amount -= amt;
    if (finalDebtors[i].amount < 0.01) i++;
    if (finalCreditors[j].amount < 0.01) j++;
  }
  ...
  if (pokerTrans.length > 0) {
    pokerTrans.forEach(function(t) {
      html += '<p style="font-size: 0.9em; padding: 5px; background: #2a2a40; margin: 3px 0; border-radius: 4px;">' + t.from + ' → ' + t.to + '<br><strong style="color: #f39c12;">
  document.getElementById('nightDetailModal').classList.add('active');
}
 ...
 + t.amount.toFixed(2) + '</strong></p>';
  ...
  if (expenseTrans.length > 0) {
    expenseTrans.forEach(function(t) {
      html += '<p style="font-size: 0.9em; padding: 5px; background: #2a2a40; margin: 3px 0; border-radius: 4px;">' + t.from + ' → ' + t.to + '<br><strong style="color: #2ecc71;">
  document.getElementById('nightDetailModal').classList.add('active');
}
...
loadData();
 + t.amount.toFixed(2) + '</strong></p>';
  ...
  html += '<p style="font-size: 0.8em; color: #aaa; text-align: center; margin-bottom: 8px;">' + optimized.length + ' payment' + (optimized.length !== 1 ? 's' : '') + '</p>';
  if (optimized.length > 0) {
    optimized.forEach(function(t) {
      html += '<p style="font-size: 0.9em; padding: 6px; background: #2a2a40; margin: 3px 0; border-radius: 4px; border-left: 2px solid #9b59b6;">' + t.from + ' → ' + t.to + '<br><strong style="color: #9b59b6;">
  document.getElementById('nightDetailModal').classList.add('active');
}
